...
| Primary buyer or user | Mobile product reviewers, educators, workshop creators, and SwiftUI teams. |
| Value wedge | Native simulation app showing motion telemetry, scenario replay, and calm mobile craft. |
...
| First action | Open the portfolio-fit note, confirm the successor project, then inspect CLI smoke passes only if this domain is relevant. |
...
- Optional entry: Paid educational app prototype ($2-$5 app) when this archived proof exactly matches the buyer problem.
- Revival step: Workshop demo license ($500-$2k workshop license) only after the active successor and support boundary are clear.
- Longer-term path: Native UX case-study package ($2k-$8k UX prototype engagement) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Scenario completions only as a discussion metric until fresh validation exists.
